...
前文说到树模型不稳定，纳闷怎么减少模型的variance？假设有$$n$$个独立变量$$Z_{1}$$、$$Z_{2}$$、……、$$Z_{n}$$，每个变量的方差都为$$\sigma^{2}$$，那么变量$$\bar{Z}$$的方差为$$\sigma^{2}/n$$，可以看到均化可以减少方差。Bagging的思想就是在原数据集中进行多次抽样得到多个训练子集，再在这些训练子集上分别训练多个模型，最后把这些模型的预测结果均化即可。
...
在之前所讲过的一系列模型中，对于优化问题，有一种通用解法就是梯度下降法：
...
仅仅受表达形式上的启发，就可以很容易想到：把模型$$f(x)$$当做需要优化的参数，把每一轮的新模型$$b(x)​$$当做负梯度，那么就可以借鉴梯度下降法的思想来得到一个最优或次优模型，由此引出**梯度提升机**(Gradient Boosting Machine)的概念。

在增量boosting算法第$$m$$轮时，令当前的损失梯度为：

$$
g_{i,m}=\frac{\partial L(y_{i},f_{m-1}(x_{i}))}{\partial f_{m-1}(x_{i})}
$$

训练一个新模型去拟合负梯度，那么GBM算法的优化问题可以写成：

$$
(\beta_{m},\theta_{m})=\arg\min\limits_{\beta,\theta}\sum\limits_{i=1}^{n}L(g_{i,m},\beta_{m}b_{m}(x_{i};\theta_{m}))
$$

特别地，当每轮的基模型使用树模型时，这种GBM被称为**梯度提升树**(Gradient Boosted Decision Trees)
