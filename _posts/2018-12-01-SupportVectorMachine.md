...
$$
Loss_{SVM}=max(0,1-y(x\theta+\theta_{0}))
$$
...
\frac{\partial{L}}{\partial{\theta}}&=-\sum_{i=1}^{m}\lambda_{i}y^{i}=0 \\
...
$$
\lambda^{*}=\arg\max\limits_{\lambda}L(\theta^{*},\lambda)
$$
...
$$
\begin{aligned}
min \ \frac{1}{2}||\theta||_{2}+C\sum_{i=1}^{m}\xi_{i}, \qquad s.t.& \  y^{(i)}(x^{(i)}\theta^{T})\ge1-\xi_{i},i=1,...m \\
& \  \xi_{i}\ge0,i=1,...m \\
\end{aligned}
$$
...
$$
\begin{aligned}
\max\limits_{\lambda,\gamma}\ \min\limits_{\theta,\xi} L(\theta,\xi,\lambda,\gamma)&=\frac{1}{2}||\theta||_{2}^{2}+C\sum\limits_{i=1}^{m}\xi_{i}-\sum_{i=1}^{m}\lambda_{i}(y^{i}\cdot{}x^{i}\theta^{T}-1+\xi_{i})-\sum_{i=1}^{m}\gamma_{i}\xi_{i}
\end{aligned}
$$

令$$\frac{\partial{L}}{\partial{\theta}}=\frac{\partial{L}}{\partial{\xi_{i}}}=0$$得：
...
$$
\begin{aligned}
\max\limits_{\lambda}\ L(\lambda)=\sum_{i=1}^{m}\lambda_{i}-\frac{1}{2}\sum_{i=1}^{m}\sum_{j=1}^{m}\lambda_{i}\lambda_{j}y^{i}y^{j}x^{i}{x^{j}}^{T} \qquad s.t.& \ 0\le\lambda_{i}\le{C},i=1,2,...,m
\end{aligned}
$$
